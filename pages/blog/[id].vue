<template>
    ...
    <div class="my-[200px]">
        <ul class="mx-auto w-[75%] text-center grid lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1 max-sm:grid-cols-1 gap-3">
        ...
        <ul class="mt-[50px] w-[170px] mx-auto text-center grid grid-cols-3 gap-2">
            <li
                class="cursor-pointer aspect-square rounded-full p-2 border border-gray-400 grid justify-center items-center">
                <NuxtLink :to="parseInt(pageNumber) - 1 == 0 ? '' : `/blog/${parseInt(pageNumber) - 1}`">{{
                    parseInt(pageNumber) - 1 == 0 ? "-" : parseInt(pageNumber) - 1 }}</NuxtLink>
            </li>
            <li
                class="cursor-pointer aspect-square rounded-full p-2 border border-gray-400 grid justify-center items-center">
                {{ pageNumber }}
            ...
            <li
                class="cursor-pointer aspect-square rounded-full p-2 border border-gray-400 grid justify-center items-center">
                <NuxtLink
                    :to="parseInt((3 * parseInt(pageNumber)) >= rawPosts.length) ? '' : `/blog/${parseInt(pageNumber) + 1}`">
                    {{ parseInt((3 * parseInt(pageNumber))) >= rawPosts.length ? "-" : parseInt(pageNumber) + 1 }}</NuxtLink>
            </li>
        </ul>
    </div>
</template>
